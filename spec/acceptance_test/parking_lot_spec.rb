describe 'Park a car when' do
  ...
  it 'registration number is valid ' do
    ...
    end.to output("#{SUCCESS_PARK_CAR} A10\n").to_stdout_from_any_process
  end
  it 'registration number is already parked ' do
    expect do
      system('MODE="test" ruby app.rb -p UP12345678')
    end.to output("#{ERR_CAR_ALREADY_PARKED} at A10\n").to_stdout_from_any_process
  end
  it 'no slot is available' do
    ...
    end.to output("#{ERR_NO_EMPTY_SLOTS}\n").to_stdout_from_any_process
  end
end
...
describe 'Invoices' do
  ...
  it 'get all' do
    ...
    end.to output("#{ParkingLotContants.INVOICE_PRINT_FORMAT(invoice_id: 'IN123456789', registration_no: 'UP12345678',
...
describe 'parking lot' do
  ...
  it 'show all cars' do
    ...
    end.to output("#{ParkingLotContants.CARS_PRINT_FORMAT([{ registration_no: 'UP12345678',
                                                             slot_id: 'A10' }])}\n").to_stdout_from_any_process
  end
end
