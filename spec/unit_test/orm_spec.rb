...
RSpec.describe CustomOrm do
  ...
  it 'read the db' do
    expect(CustomOrm.read_db_file(TEST_DB_DIR,
  ...
  it 'write the items' do
    ...
    CustomOrm.write_db_file(TEST_DB_DIR, db_name: DB_CARS, data: data)

    expect(CustomOrm.read_db_file(TEST_DB_DIR, db_name: DB_CARS)).to eql data
  end
  it 'delete the item with key' do
    CustomOrm.delete_element_by_key(TEST_DB_DIR, db_name: DB_CARS, key: 'UP12345678')
    expect(CustomOrm.read_db_file(TEST_DB_DIR, db_name: DB_CARS)).to eql({})
  end

  it 'delete the last item' do
    CustomOrm.delete_element_by_key(TEST_DB_DIR, db_name: DB_CARS,
                                                 key: 'UP12345678')
    expect(CustomOrm.read_db_file(TEST_DB_DIR, db_name: DB_CARS)).to eql({})
  end
end
